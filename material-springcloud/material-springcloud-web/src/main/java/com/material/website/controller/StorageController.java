...
public class StorageController {
	...
	public String addInit(Integer type,Model model){
		Integer count = storageFeign.getStorageCount(type);
		if(type == 1){
	...
	public Map<String, Object> lockStorageInfo(Integer storageId){
		...
		boolean isSuccess = storageFeign.addLockStorage(storageId);
		if(isSuccess){
			map.put("status", 200);
			map.put("msg", "入账成功");
		}else{
	...
	public String updateStorageInit(Integer storageId,Integer storageType,Model model){
		Map<String,Object>resultMap = storageFeign.updateStorageInit(storageId);
	...
	public String queryStorageGoods(Integer id,String supplierName,Double sumMoney,String operatNo,String returnPage,Model model) throws UnsupportedEncodingException{
		...
		List<GoodsInstallDto>resultList = storageFeign.queryGoodsById(id);
	 ...
	 */
	@RequestMapping(value="/queryStorageByNo",method={RequestMethod.POST,RequestMethod.GET})
	@ResponseBody
	public Map<String, Object> queryStorageByNo(String operatNo) throws ParseException{
        ...
        Storage storage = storageFeign.queryStorageByNo(operatNo);

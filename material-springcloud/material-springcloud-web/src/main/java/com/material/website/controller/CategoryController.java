...
public class CategoryController {
	...
	public List<CategoryDto> queryCategoryList(Integer pid){
		...
		try {
			resultList = categoryFeign.queryCategoryList(pid);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return resultList;
	...
	public String addSupplier(CategoryArgs categoryArgs,Model model){
		...
		if(categoryArgs.getParentName().equals("类目树")){
		   ...
		   List<CategoryDto>  list = categoryFeign.queryCategoryList(categoryArgs.getParentId());
		   Category category = categoryFeign.loadCategory(categoryArgs.getParentId());
	...
	public String updateInit(Integer categoryId,Model model){
		...
		Category category = categoryFeign.loadCategory(categoryId);
		if(category == null){
			model.addAttribute("errorInfo","初始化出错！");
			return "admin/category/update";
		}
		model.addAttribute("category",category);
		Category parentCategory = categoryFeign.loadCategory(category.getParentId());
	...
	public Map<String, Object> delCategory(Integer categoryId){
		Map<String, Object>map = categoryFeign.delCategory(categoryId);
		return map;
	}
	...
	public String createCategoryNo(Integer parentId,String parentName,String categoryName){
		try {
			...
			if(parentName.equals("类目树")){
				...
				Category category = categoryFeign.loadCategory(parentId);
			   List<CategoryDto>  list = categoryFeign.queryCategoryList(parentId);
			   if(list == null || list.size() <= 0){
				   categoryNo = category.getCategoryNo()+"01";
			   }else{
				  categoryNo = category.getCategoryNo()+MaterialNoUtil.getNo(2, list.size()+1); 
